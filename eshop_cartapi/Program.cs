...
builder.Services.AddDbContext<eshop_cartapiContext>(options => options.UseLazyLoadingProxies().UseSqlServer(builder.Configuration.GetConnectionString("DefaultConnection")));
builder.Services.AddIdentity<ApplicationUser, IdentityRole>().AddEntityFrameworkStores<eshop_cartapiContext>().AddDefaultTokenProviders();
